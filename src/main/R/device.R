    ...
    function(which = dev.next())
{
    ...
    which <- fxdev.set(as.integer(which))
    ...
    function(which = dev.cur())
{
    ...
    num.device <- fxdev.next(as.integer(which))
    names(num.device) <- .Devices[[num.device]]
    num.device
}
    ...
    function(which = dev.cur())
{
    ...
    num.device <- fxdev.prev(as.integer(which))
    names(num.device) <- .Devices[[num.device]]
    num.device
}
    ...
    function(which = dev.cur())
{
    ...
    .Internal(fxdev.off(as.integer(which)))
    dev.cur()
}
...
dev.copy <- function(device, ..., which = dev.next())
{
    ...
    .Internal(fxdev.copy(old.device))
    on.exit()
    dev.cur()
}
...
dev.control <- function(displaylist = c("inhibit", "enable"))
{
	...
	.Internal(fxdev.control(displaylist == "enable"))
...
dev.new <- function()
{
    ...
    num.device <- fxstage()
    #assign(".Devices[num.device]","fx",envir=.GlobalEnv)
    .Devices[num.device] <<- "fx"
    names(num.device) <- .Devices[[num.device]]
    return(num.device)
}
